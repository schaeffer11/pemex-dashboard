...
router.get('/incProdData', (req, res) => {
  ...
  let query = 
`SELECT ${select} fwm.SUBDIRECCION_NAME, fwm.ACTIVO_NAME, fwm.FIELD_NAME, fwm.WELL_FORMACION_ID, FORMACION, COMPANY, WELL_NAME, tr.FECHA_INTERVENCION, YEAR(tr.FECHA_INTERVENCION) as YEAR, MONTH(tr.FECHA_INTERVENCION) as MONTH, TIPO_DE_TERMINACION, TIPO_DE_INTERVENCIONES, SUM(QO_RESULT) AS QO_RESULT, SUM(QG_RESULT) AS QG_RESULT
  FROM Results 
...
SUM(IF(re.VOLUMEN_GEL_LINEAL, re.VOLUMEN_GEL_LINEAL, 0) + IF(ra.VOLUMEN_GEL_LINEAL, ra.VOLUMEN_GEL_LINEAL, 0) + IF(rap.VOLUMEN_GEL_LINEAL, rap.VOLUMEN_GEL_LINEAL, 0)) as TOTAL_GEL_LINEAL,
...
+ IF(re.VOLUMEN_GEL_DE_FRACTURA, re.VOLUMEN_GEL_DE_FRACTURA, 0)
+ IF(ra.VOLUMEN_GEL_DE_FRACTURA, ra.VOLUMEN_GEL_DE_FRACTURA, 0)
+ (IF(rap.VOLUMEN_PRECOLCHON_APUNTALANTE, rap.VOLUMEN_PRECOLCHON_APUNTALANTE, 0)/  264.172)
+ IF(ra.VOLUMEN_GEL_LINEAL, ra.VOLUMEN_GEL_LINEAL, 0)
+ IF(re.VOLUMEN_GEL_LINEAL, re.VOLUMEN_GEL_LINEAL, 0)
+ IF(rap.VOLUMEN_GEL_LINEAL, rap.VOLUMEN_GEL_LINEAL, 0)
+ IF(ra.VOLUMEN_MODIFICADOR_PERMEABILIDAD, ra.VOLUMEN_MODIFICADOR_PERMEABILIDAD, 0)
+ IF(re.VOLUMEN_MODIFICADOR_PERMEABILIDAD, re.VOLUMEN_MODIFICADOR_PERMEABILIDAD, 0)
+ IF(ra.VOLUMEN_ESPACIADOR, ra.VOLUMEN_ESPACIADOR, 0)
+ IF(re.VOLUMEN_ESPACIADOR, re.VOLUMEN_ESPACIADOR, 0)
) as TOTAL_VOLUME,
SUM(
