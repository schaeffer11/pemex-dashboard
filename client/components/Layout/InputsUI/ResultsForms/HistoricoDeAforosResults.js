...
import { setGeneralHistoricoDeAforosResults } from '../../../../redux/actions/results'
...
@autobind class HistoricoDeAforos extends Component {
  ...
  componentDidMount(){
    const { setGeneralHistoricoDeAforosResults, hasSubmitted } = this.props

    if (hasSubmitted) {
      const hasErrors = this.checkAllInputs()
      setGeneralHistoricoDeAforosResults(['hasErrors'], hasErrors)
    }
  }

  componentDidUpdate(prevProps) {
    let { hasSubmitted, formData, setGeneralHistoricoDeAforosResults } = this.props
    // formData = formData.toJS()
    let { fromSave } = formData
    
    if (hasSubmitted !== prevProps.hasSubmitted || fromSave) {
      let err = this.checkAllInputs(true)
      setGeneralHistoricoDeAforosResults(['hasErrors'], err)
      if (fromSave === true) {
        setGeneralHistoricoDeAforosResults(['fromSave'], false)
      }
    }
  }
  ...
  checkForErrors(value) {
    ...
    this.setState({ errors: errorsCopy }, () => {
      const { setGeneralHistoricoDeAforosResults } = this.props
      const hasErrors = this.checkAllInputs()
      setGeneralHistoricoDeAforosResults(['hasErrors'], hasErrors)
    })
  }
  ...
  makeHistoricoDeAforosInput() {
    let { formData, setGeneralHistoricoDeAforosResults, hasSubmitted } = this.props
    // formData = formData.toJS()
    ...
    return (
      <div className='historico-produccion' >
        <ExcelUpload
            ...
            setData={d => setGeneralHistoricoDeAforosResults(['aforosData'], d)}
          />
        <div className='table'>
          <InputTable
            className="-striped"
...
const mapStateToProps = state => ({
  formData: state.get('historicoDeAforosResults').toJS(),
  hasErrors: state.getIn(['historicoDeAforosResults', 'hasErrors']),
...
const mapDispatchToProps = dispatch => ({
    ...
    setGeneralHistoricoDeAforosResults: (location, value) => dispatch(setGeneralHistoricoDeAforosResults(location, value)),
})
