...
export const Bar = ({ estimated, actual }) => {
  ...
  return (
    <div className={classnames("Bar")} style={{height: '80%', paddingLeft: '30px', paddingRight: '30px'}}>
      <div style={{textAlign: 'left'}}> Producción Incremental Real: <span style={{color: '#0D547B', fontSize: '20px', fontWeight: 'bold'}}> {actual ? `${numWithCommas(actual.toFixed(0))} bbl/d` : null} </span> </div>
      <div style={{height: '17%', background: realColor, width: realWidth }} />
      <div style={{textAlign: 'left', paddingTop: '15px'}}> Producción Incremental Estimada: <span style={{color: '#0D547B', fontSize: '20px', fontWeight: 'bold'}}> {estimated ? `${numWithCommas(estimated.toFixed(0))} bbl/d` : null} </span> </div>
